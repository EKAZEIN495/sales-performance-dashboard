...
5. Buka **Overview → Connection information** dan salin **Service URI**. Database awal biasanya `defaultdb`.
...
```env
DATABASE_URL="mysql://avnadmin:PASSWORD@HOST:PORT/defaultdb?sslcert=ca.pem&sslaccept=strict&connection_limit=3"
```
...
1. Buat backup dari XAMPP:
   ...
   ```powershell
   & "C:\xampp\mysql\bin\mysqldump.exe" -u root --single-transaction --no-create-info --result-file="erafone-data.sql" erafone Store Sales Target DailyEntry
   ```
...
5. Pastikan dashboard berubah langsung atau maksimal 10 detik.
6. Pulihkan nilai uji dan coba tombol **Export Excel**.
...
- Log error berada di Netlify **Logs → Functions**.
...
- Aiven Free menyediakan 1 GB storage dan maksimal 76 koneksi. `connection_limit=3` membantu menjaga penggunaan koneksi Prisma tetap kecil.
